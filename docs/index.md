...
- ダウンロード先: [GitHub (jogu6/imagededuper)](https://github.com/jogu6/imagededuper)
...
| --- | --- |
| `JPEG / JPG` | ???????? JPEG ???JPEG/JPG ??????????????????? / Standard JPEG images; both `.jpeg` and `.jpg` extensions are handled directly. |
| `PNG` | ??????? PNG ????????????????? / PNG files (including transparency) are compared as-is. |
| `BMP` | ?????????????????????????????????? / Lossless BMP files are loaded and processed through the same pipeline. |
| `GIF` | ??? GIF?????????????????? / Still GIFs (first frame) are analyzed for duplicates. |
| `TIFF` | TIFF/TIF ?????????????????? / TIFF/TIF files are supported with extension correction when needed. |
| `WebP` | `webp` ?????????????????????????? / WebP images are loaded directly and compared with other formats. |
| `JFIF` | `.jfif` ????????? `.jpg` ???????????????????? / `.jfif` files are automatically renamed to `.jpg` before comparison. |
| `HEIC / HEIF` | HEIC/HEIF ? JPEG ????????????? `duplicates/` ??????? / HEIC/HEIF images are converted to JPEG automatically, with originals moved to `duplicates/`. |
